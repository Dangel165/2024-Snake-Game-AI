...
### 🧩 **미로 시스템 추가**
- **새로운 미로 모드**: M키로 장애물 ON/OFF 토글 가능
- **다양한 장애물 패턴**:
  - 십자 모양 중앙 장애물
  - 모서리 L자 장애물 (4개 코너)
  - 랜덤 배치 장애물 (10개)
- **스마트 배치**: 시작 위치와 중앙 근처는 장애물 생성 방지

### ⚡ **속도 조절 시스템**
- **실시간 속도 조절**: +/- 키로 게임 속도 조절
- **속도 범위**: 10 ~ 100 (기본값: 40)
- **시각적 피드백**: 화면에 현재 속도 표시

### 🤖 **AI 학습 시스템 개선**
- **확장된 상태 공간**: 11차원 → 19차원
  - 기존 11개: 충돌 감지, 방향, 음식 위치
  - 새로운 8개: 장애물 감지 (직선 4개 + 대각선 4개)
- **적응형 학습**: 미로 모드에 따른 동적 상태 조정
- **향상된 신경망**: 19개 입력을 처리하는 더 큰 네트워크

### 🧑‍💻 멀티플레이어 AI 기능 추가
모든 플레이어가 독립적인 AI: 랜덤 행동 제거, 최대 4명의 AI가 동시 경쟁
플레이어 간 충돌 감지: AI끼리 부딪히면 게임 오버
개별 모델 저장: 각 플레이어별 독립적인 모델 

### 🐛 발견된 버그수정

1.멀티플레이어 모드에서 뱀이 움직이지 않는 문제 수정

2.멀티 모드를 키고 끌떄 그냥 프로그램이 강제로 꺼지는 버그 수정
